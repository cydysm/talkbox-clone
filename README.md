...
- Django Admin 管理文章、分类、评论、媒体、导航链接和独立页面；管理首页按「内容管理 / 站点配置 / 互动管理」分组
- 后台「站点设置」统一管理站点名称、网页标题、meta 描述、主页简介、Favicon 与「显示原文按钮」开关
- 未上传 Favicon 时自动回退到当前主题的仙人掌 logo
- 文章与页面别名可留空，自动从标题生成拼音 slug（重复自动去重）
- Markdown 文章渲染和 Redis 渲染缓存
- 多标签（django-taggit）
- 树形评论模型和评论审核状态；评论邮箱可留空
- 评论与回复邮件提醒
- 图片上传与缩略图生成
...
- 作者与后台用户可预览草稿，访客不可访问
- 评论蜜罐防护和 IP 提交间隔限制
- 操作消息提示 10 秒倒计时后自动消失（悬停暂停）
- `/healthz/` 应用、数据库和缓存健康检查
- 仿 Cactus 主题模板（深色/浅色两套），移动端断点统一为 500px
- 分类页、标签页、存档页和站内搜索
- 存档页按年份分组列出全部文章，并附分类/标签一览（含计数）
- 文章列表统一分页，每页数量可配置
- RSS 与 Atom 订阅源（标题与描述跟随后台站点设置）
- 后台多模板切换，当前支持 Cactus Dark 和 Cactus Light
- 访客可切换深/浅色或跟随系统，偏好保存在 Cookie 中
- 文章页固定导航：上一篇/下一篇、回到顶部和分享面板（Facebook/X/LinkedIn/Reddit/HN/邮件）；顶部导航与菜单按钮同步展开/收起
- 文章支持 Markdown 原文视图（`?view=markdown`），可在站点设置中关闭
- 后台可管理站点导航链接（数量上限可配置），支持按位置隐藏（如仅首页显示、不进文章页顶部导航）
- 后台可编辑独立页面，发布后以顶级路径访问（如 `/about/`），采用专属页面版式（标题 + 更新时间 + 分隔线）
...
管理首页顶部是内容统计卡片（文章总数/浏览量/待审核评论）和最新待审核评论，下方条目按「内容管理 / 站点配置 / 互动管理」三组呈现。插件设置的行由插件注册表生成，后台只提供启用/禁用开关，不提供手工增删。
...
后台 **站点设置** 是单例配置，一处管理站点的对外展示：

- **站点名称（显示）**：页面顶部大标题、页脚版权和 RSS 链接名称；留空用 `SITE_NAME`
- **网页标题**：浏览器标签页 `<title>` 与 RSS 标题；留空用 `SITE_NAME`
- **站点描述（meta）**：`meta description`；留空用 `SITE_DESCRIPTION`
- **主页简介**：主页顶部文案，最长 200 字；留空用站点描述
- **显示开关**：如「显示原文按钮」
- **Favicon**：上传 PNG/ICO；留空时用当前主题的仙人掌 logo

### 站点导航与独立页面

后台 **导航链接** 管理站点导航，每项包含标题、链接（站内路径或完整 URL）和排序，最多同时显示 8 个，可在 `.env` 用 `NAV_MAX_ITEMS` 调整；没有任何导航项时自动显示「首页」。每项还有两个作用域控制：

- **显示范围**：所有页面 / 仅主页 / 仅其它页面 / 隐藏
- **显示在文章页顶部导航**：关闭后该条目不出现在文章页右上角导航（如友链这类低频页只放首页和页脚）

后台 **独立页面** 用于创建非文章内容（如「关于」）：填写标题、路径（如 `about`）和 Markdown 内容，发布后通过顶级路径访问（如 `/about/`），采用专属页面版式（标题 + 更新时间 + 分隔线），与首页文章流、文章详情区分。路径不能使用系统保留字（`search`、`post`、`archive` 等）；草稿仅 staff 可预览。页脚会在导航项之后固定附带 RSS 链接。

### 内容浏览

公开页面包括：

- `/archive/`：存档页，按年份分组列出全部已发布文章，并附分类/标签一览（含文章数计数）
...
评论表单包含对普通用户隐藏的蜜罐字段。昵称和内容必填，邮箱可留空（留空时被回复的通知会静默跳过）。同一 IP 默认每 `COMMENT_INTERVAL_SECONDS=30` 秒只能提交一次，可在 `.env` 调整。
...
所有核心功能已完成：博客内容管理、评论树（邮箱可留空）、媒体上传与水印、多主题切换（含访客偏好）、统一站点设置（名称/标题/描述/简介/Favicon）、分组式管理后台、存档页、按位置作用域的导航管理、通用导入器、插件依赖隔离、登录限速、异步邮件通知、一键升级/恢复、CI 构建验证与依赖安全审计。
